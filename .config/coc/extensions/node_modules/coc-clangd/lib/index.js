...
async function activate(context) {
    ...
    context.subscriptions.push(status, coc_nvim_1.commands.registerCommand('clangd.switchSourceHeader', cmds.switchSourceHeader(ctx)), coc_nvim_1.commands.registerCommand('clangd.symbolInfo', cmds.symbolInfo(ctx)), coc_nvim_1.workspace.onDidOpenTextDocument(() => {
        status.updateStatus();
    }));
...
class SemanticHighlightingFeature {
    ...
    async handleNotification(params) {
        ...
        const lines = params.lines.map((line) => ({ line: line.line, tokens: this.decodeTokens(line.tokens) }));
        const symbols = [];
        const skipped = [];
        for (const line of lines) {
            for (const token of line.tokens) {
                if (token.kind === 'InactiveCode') {
                    skipped.push(vscode_languageserver_protocol_1.Range.create(line.line, token.character, line.line, token.character + token.length));
                }
                else {
                    symbols.push({
                        id: 0,
                        kind: token.kind,
                        ranges: [vscode_languageserver_protocol_1.Range.create(line.line, token.character, line.line, token.character + token.length)],
                        parentKind: 'Unknown',
                        storage: 'None',
                    });
                }
